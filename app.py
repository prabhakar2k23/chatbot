...
def store_chat_history(user_message, bot_response):
    ...
    try:
        ...
        check_query = "SELECT id FROM chathistory WHERE user_message = %s LIMIT 1"
        cursor.execute(check_query, (user_message,))
        existing_entry = cursor.fetchone()
        if existing_entry:
            print("Duplicate message detected. Not inserting again.")  
            return  
        # Check if the reply alredy exists
        check_query1 = "SELECT id FROM chathistory WHERE user_message = %s LIMIT 1"
        cursor.execute(check_query1, (bot_response))
        existing_entry1 = cursor.fetchone()
        if existing_entry1:
            print("Duplicate reply detected. Not inserting again.")  
            return
